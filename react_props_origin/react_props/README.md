...
This project focuses on React component composition and props management. You'll learn to break down a monolithic application into reusable components, pass data between components using props, and enhance component behavior through proper testing.
...
- Check whether the `li` element has the color `blue`, and the attribute `data-notification-type` set to `default` whenever the component receives the prop's `type` as "default".
- Check whether the `li` element has the color `red`, and the attribute `data-notification-type` set to `urgent` whenever the component receives the props's `type` as "urgent".
...
- Add new tests to check the behavior of the UI whenever the prop `displayDrawer` set to `true`:
